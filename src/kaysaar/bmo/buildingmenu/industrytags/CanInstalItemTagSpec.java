...
public class CanInstalItemTagSpec extends IndustryTagSpec{
    public CanInstalItemTagSpec(String tag, String tagName, ArrayList<String> specsManuallyPlaced, IndustryTagType type) {
        ...
        for (IndustryDropDownButton industryDropDownButton : buttonsExisting) {
            for (IndustrySpecAPI o : industryDropDownButton.getSpecs()) {
                if(specs.contains(o.getId())){
                    ...
                    if(!BuildingMenuMisc.getItemsForIndustry(ind.getSpec().getId(),false).isEmpty()){
