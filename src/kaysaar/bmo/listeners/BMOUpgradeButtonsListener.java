...
public class BMOUpgradeButtonsListener implements IndustryOptionProvider {
    ...
    public void optionSelected(IndustryOptionData opt, DialogCreatorUI ui) {
        if (opt.id == AOTD_VOK_UPGRADE) {
            CustomDialogDelegate delegate = new UpgradeListUI(opt.ind,new ArrayList<>( BuildingMenuMisc.getUpgradeChildren(opt.ind.getId())));
    ...
    public IndustryOptionData validateUpgradeOption(Industry ind){
        ...
        if(!BuildingMenuMisc.getUpgradeChildren(ind.getSpec().getId()).isEmpty()){
            boolean canShow = false;
            for (String s :BuildingMenuMisc.getUpgradeChildren(ind.getSpec().getId())) {
